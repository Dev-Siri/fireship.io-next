...
{{< figure src="/img/content/snippets/meilisearch-create-index.png" >}}
...
```javascript
exports.meilisearchIndex = functions.firestore.document("movies/{id}").onCreate(async (snapshot, context) => {
  const index = client.getIndex("movies");

  const id = snapshot.id;
  const { title, year, description } = snapshot.data();

  const response = await index.addDocuments([{ id, title, year, description }]);
...
{{< figure src="/img/content/snippets/gcp-disk.png" caption="Create a disk on Compute Engine" >}}
...
{{< figure src="/img/content/snippets/meilisearch-google-cloud-settings.png" caption="Important VM settings" >}}
...
{{< figure src="/img/content/snippets/meilisearch-keys.png" caption="Make a request for your API keys" >}}
