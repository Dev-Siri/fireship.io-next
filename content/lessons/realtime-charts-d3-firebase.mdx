...
{{< figure src="/img/content/snippets/d3-firestore-chart.png" caption="Example of the realtime chart you will build in this tutorial" >}}
...
const svg = d3.select(".canvas").append("svg").attr("width", svgWidth).attr("height", svgHeight).style("border", "2px solid gray"); // Chart border
...
```javascript
const chart = svg.append("g").attr("width", chartWidth).attr("height", chartHeight).attr("transform", `translate(${margin.left}, ${margin.top})`);
