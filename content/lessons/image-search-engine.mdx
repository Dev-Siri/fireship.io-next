...
{{< figure src="/img/content/snippets/image-search-vector-db.png" caption="Image search in a vector database" >}}
...
```javascript
const img = readFileSync(".//img/content/snippets/hi-mom.jpg");
...
const resImage = await client.graphql.get().withClassName("Meme").withFields(["image"]).withNearImage({ image: test }).withLimit(1).do();
