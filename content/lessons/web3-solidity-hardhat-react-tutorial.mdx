...
{{< figure src="/img/content/snippets/ipfs-folder.png" caption="Notice the unique Content ID in the IPFS folder" >}}
...
{{< figure src="/img/content/snippets/open-zeppelin-wizard.png" caption="Use the OpenZeppelin wizard to create a base contract" >}}
...
<File name="scripts/sample-script.js">
  <JavaScript />
</File>
...
{{< figure src="/img/content/snippets/hardhart-deploy-contract.png" caption="Make a note of deployed contract address" >}}


## Web3 Frontend

{{< figure src="/img/content/snippets/nft-demo.png" caption="Web3 NFT demo app" >}}
...
function NFTImage({ tokenId, getCount }) {
  ...
  return (
    <div>
      <img src={isMinted ? imageURI : "/img/content/snippets/placeholder.png"}></img>
      <h5>ID #{tokenId}</h5>
      {!isMinted ? <button onClick={mintToken}>Mint</button> : <button onClick={getURI}>Taken! Show URI</button>}
    </div>
  );
}
