<script lang="ts">
  import { canAccess, modal, userProgress } from "../../../stores";
  import { markComplete, markIncomplete } from "../../../utils/firebase";
...
<span class="wrap">
  {#if free || $canAccess}
    ...
    {:else}
      ...
      <span class="msg" class:pink={quiz}>{quiz ? "pop quiz" : "incomplete"}</span>
    {/if}
  {:else}
    <modal-action name="signin" type="open" class="purple">
      <svg xmlns="http://www.w3.org/2000/svg" class="svg-sm" viewBox="0 0 448 512"
        ><path
          fill="currentColor"
