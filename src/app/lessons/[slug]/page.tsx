  ...
  return (
    <main className="container">
		...
		<article itemScope itemType="http://schema.org/Article" className="w-full">
			...
			<header>
				{/* Video player */}
				{youtube && <div itemProp="video" itemScope itemType="http://schema.org/VideoObject">
        ...
        <div className="bg-gray6 py-5 px-3 rounded-lg mb-4 flex justify-between mt-3">
					...
					<Link className="flex items-center no-underline" itemProp="author" aria-label={author} href="/contributors/{{ anchorize .Params.author }}/">
						...
						<span className="flex flex-col ml-2" >
							...
							<span className="text-xs text-gray4">
								Posted{" "}
								<time itemProp="dateModified">{formatDate(datemod)}</time>
						</span>
					</a>
					{/* Tags */}
					<span>
						{tags.map(tag => (
            	<Link href={`/tags/${tag}`}><span className={`tag tag-${tag}`}>#{tag}</span></Link>
            ))}
						{github && (
